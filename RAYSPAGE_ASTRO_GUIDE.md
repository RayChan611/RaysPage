...
- **光标**：桌面自定义光标（dot 即时跟随 + outline lerp 0.12 缓动跟进），移动端（≤768px）禁用。
...
### 5.4 自定义光标 & 粒子背景
- **光标**：仅桌面 `innerWidth>768`，CSS `@media (max-width:768px)` 也 `display:none`。`cursor.js` 给 `body` 加 `.custom-cursor-active`（CSS 里 `.custom-cursor-active *{cursor:none}` 隐藏系统光标）。dot 用 `pointermove` 立即定位；outline 用 lerp 0.12 跟随，接近时停 RAF 省 CPU。hover 到 `a/button/.contact-card/.tag/.social-link/.gallery-item/.essay-card` 时放大。
